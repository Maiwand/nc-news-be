...
describe("seed", () => {
  ...
  describe("saved_articles table", () => {
    test("saved_articles table exists", () => {
      return db
        .query(
          `SELECT EXISTS (
    ...
    test("saved_articles table has username column of varying character", () => {
      return db
        .query(
          `SELECT column_name, data_type
       FROM information_schema.columns
    ...
    test("saved_articles table has article_id column as integer", () => {
      return db
        .query(
          `SELECT column_name, data_type
       FROM information_schema.columns
  ...
  describe("user_topics table", () => {
    ...
    test("user_topics table has topic_slug column of varying character", () => {
      return db
        .query(
          `SELECT column_name, data_type
       FROM information_schema.columns
       ...
       AND column_name = 'topic_slug';`
        ...
        .then(({ rows: [column] }) => {
          expect(column.column_name).toBe("topic_slug");
          expect(column.data_type).toBe("character varying");
        });
    });
  });
...
describe("data insertion", () => {
  ...
  test("saved_articles data has been inserted correctly", () => {
    return db.query(`SELECT * FROM saved_articles;`).then(({ rows: saved }) => {
  ...
  test("user_topics data has been inserted correctly", () => {
    return db.query(`SELECT * FROM user_topics;`).then(({ rows: entries }) => {
      expect(entries.length).toBeGreaterThan(0);
      entries.forEach((entry) => {
        expect(entry).toHaveProperty("username");
        expect(entry).toHaveProperty("topic_slug");
      });
    });
  });
